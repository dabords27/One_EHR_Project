            ...
            <div className="flex items-center gap-3">
              ...
              {!isCollapsed && (
                <div className="flex flex-col animate-in fade-in slide-in-from-left-2 duration-300">
                  <span className="font-black text-white tracking-tighter text-sm leading-none uppercase">ONE EHR</span>
                  <span className="text-[7px] font-black text-sky-400 uppercase tracking-[0.2em] mt-1">HEALTH RECORD SYSTEM</span>
                </div>
              )}
            </div>
            ...
            {!isCollapsed && (
              <div className="mt-8 px-1 animate-in fade-in slide-in-from-top-2 duration-300">
                <div className="flex flex-col">
                  <p className="text-[9px] text-slate-500 font-black uppercase tracking-[0.2em] leading-tight mb-1">
                    Active Station
                  </p>
    ...
    {user.fullName?.charAt(0)}
              ...
              {!isCollapsed && (
                <div className="overflow-hidden animate-in fade-in slide-in-from-left-2 duration-300">
                  ...
                  <p className="text-[9px] text-sky-400 uppercase tracking-[0.2em] font-black">{user.role}</p>
                </div>
              )}
            </div>
            <button 
              onClick={onLogout}
              className={`w-full flex items-center justify-center bg-rose-500/10 text-rose-400 hover:bg-rose-500 hover:text-white rounded-xl transition-all text-[10px] font-black uppercase tracking-[0.2em] ${
      ...
      >
        ...
        <header className="bg-white border-b border-slate-200 sticky top-0 z-50 px-8 h-[72px] flex items-center shadow-sm">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 w-full">
            <div>
              <h1 className="text-lg font-black text-slate-800 tracking-tighter leading-none mb-1 uppercase">
                Julius K. Quiambao Medical & Wellness Center Inc.
              </h1>
              <p className="text-[9px] text-slate-400 font-bold tracking-[0.2em] uppercase">
                Powered by Jayvee Palisoc
              </p>
            </div>
